...
async def seed_db() -> uuid.UUID:
    ...
    async with AsyncSessionLocal() as db:
        ...
        brand_with_cost = Brand(
            id=uuid.uuid4(),
            agency_id=agency.id,
            name=BRAND_WITH_COST_NAME,
            slug=f"e2e-prof-cost-{uuid.uuid4().hex[:8]}",
        ...
        brand_no_cost = Brand(
            id=uuid.uuid4(),
            agency_id=agency.id,
            name=BRAND_NO_COST_NAME,
            slug=f"e2e-prof-nocost-{uuid.uuid4().hex[:8]}",
            status=BrandStatus.ACTIVE.value,
            currency="TRY",
        )
        ...
        owner = User(
            ...
            full_name="E2E Profitability Owner",
        ...
        db.add_all(
            [
                ClientInvoiceLine(
                    id=uuid.uuid4(),
                    invoice_id=invoice_with_cost.id,
                    source_type="time_entry",
                    description="E2E invoiced hours (cost rate configured)",
                    quantity=5,
                    unit="saat",
                    ...
                    tax_cents=100_000,
                    total_cents=600_000,
                    billing_rate_snapshot_cents=100_000,
                    cost_rate_snapshot_cents=40_000,
                ),
                ClientInvoiceLine(
                    id=uuid.uuid4(),
                    invoice_id=invoice_no_cost.id,
                    source_type="time_entry",
                    description="E2E invoiced hours (no cost rate configured)",
                    quantity=4,
                    unit="saat",
                    unit_price_cents=100_000,
                    tax_rate_bps=2000,
