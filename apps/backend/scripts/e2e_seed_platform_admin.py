...
async def _get_or_create_platform_admin(db) -> User:
    ...
    if existing is not None:
        ...
        await db.execute(
            delete(UserMfaRecoveryCode).where(UserMfaRecoveryCode.user_id == existing.id)
        )
        return existing
    admin = User(
        id=uuid.uuid4(),
...
async def _seed_approval_token(
    db, agency_id: uuid.UUID, requester_id: uuid.UUID, title: str
) -> str:
    brief = Brief(
        id=uuid.uuid4(),
        agency_id=agency_id,
...
async def seed() -> dict[str, str]:
    async with AsyncSessionLocal() as db:
        ...
        plan_wl = Plan(
            id=uuid.uuid4(),
            code="e2e-white-label",
            name="E2E White Label Plan",
        ...
        agency_a = Agency(
            id=uuid.uuid4(),
            name="E2E Beyaz Etiket Ajansı",
            slug=f"e2e-wl-agency-{suffix}",
        ...
        agency_b = Agency(
            id=uuid.uuid4(),
            name="E2E Varsayılan Ajans",
            slug=f"e2e-default-agency-{suffix}",
            status=AgencyStatus.ACTIVE.value,
        ...
        db.add_all(
            [
                Subscription(
                    id=uuid.uuid4(), agency_id=agency_a.id, plan_id=plan_wl.id, status="active"
                ),
                Subscription(
                    id=uuid.uuid4(), agency_id=agency_b.id, plan_id=plan_starter.id, status="active"
                ),
                AgencyMember(
                    id=uuid.uuid4(),
        ...
        home_seo.description = (
            "E2E testleri için gerçekçi bir meta açıklama metni, elli karakterin üzerinde."
        )
