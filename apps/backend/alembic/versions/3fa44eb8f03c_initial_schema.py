...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_platform_audit_logs_admin_user_id'), 'platform_audit_logs', ['admin_user_id'], unique=False)
    ...
    sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('deleted_at', sa.DateTime(timezone=True), nullable=True),
    sa.CheckConstraint('(agency_id IS NOT NULL AND brand_id IS NULL) OR (agency_id IS NULL AND brand_id IS NOT NULL)', name='ck_subscription_exactly_one_tenant'),
    sa.ForeignKeyConstraint(['agency_id'], ['agencies.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['brand_id'], ['brands.id'], ondelete='CASCADE'),
