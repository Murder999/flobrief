...
function actionLabel(action: string, t: TFunction): string {
  const map: Record<string, TranslationKey> = {
    brief_created: "dashboard.activity.briefCreated",
    brief_updated: "dashboard.activity.briefUpdated",
    brief_status_changed: "dashboard.activity.briefStatus",
    brief_approved: "dashboard.activity.briefApproved",
    brief_revision_requested: "dashboard.activity.revisionRequested",
    member_invited: "dashboard.activity.memberInvited",
    member_joined: "dashboard.activity.memberJoined",
    brand_created: "dashboard.activity.brandCreated",
    calendar_item_created: "dashboard.activity.calendarCreated",
    calendar_item_updated: "dashboard.activity.calendarUpdated",
  };
  return map[action] ? t(map[action]) : action.replace(/_/g, " ");
}

function entityTypeLabel(entityType: string, t: TFunction): string {
  const map: Record<string, TranslationKey> = {
    brief: "dashboard.entity.brief", brand: "dashboard.entity.brand", calendar_item: "dashboard.entity.calendar",
    agency_member: "dashboard.entity.member", user: "dashboard.entity.user",
  };
  return map[entityType] ? t(map[entityType]) : entityType;
}

function formatRelativeTime(iso: string, locale: "en" | "tr", t: TFunction): string {
  ...
  return formatLocalizedDate(iso, locale, { day: "numeric", month: "short" });
}
      ...
      <div className="relative flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={cn(
            ...
            {typeof value === "number" ? formatLocalizedNumber(value) : value}
          </p>
  ...
  const inner = (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-lg font-bold text-text tracking-tight">{formatLocalizedNumber(value)}</p>
      </div>
    </div>
  );
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto">
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-xl font-semibold text-text mb-2">{t("dashboard.agency.notFound")}</h2>
          <p className="text-sm text-text-muted max-w-sm">
            {t("dashboard.agency.notFoundBody")}
          </p>
        </div>
      </div>
    );
      ...
      <div className="border-b border-border bg-surface px-4 sm:px-8 py-6 mb-0">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="mt-0.5 text-sm text-text-secondary">
            {activeAgency?.name && t("dashboard.agency.summary", { agency: activeAgency.name })}
          </p>
        </div>
      </div>
    <div className="p-4 sm:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {!stats || statsError ? (
          ...
          <>
            <StatCard label={t("dashboard.stats.activeBrands")} value={stats.active_brands} icon={Building2} iconClass="bg-accent-subtle text-accent" href="/dashboard/brands" />
            <StatCard label={t("dashboard.stats.activeMembers")} value={stats.active_members} icon={Users} iconClass="bg-success-subtle text-success" href="/dashboard/settings/members" />
            <StatCard label={t("dashboard.stats.openBriefs")} value={stats.open_briefs} sub={t("dashboard.stats.draftReview")} icon={FileText} iconClass="bg-warning-subtle text-warning" href="/dashboard/briefs" />
            <StatCard label={t("dashboard.stats.awaitingApproval")} value={pendingBriefs?.length ?? 0} sub={t("dashboard.stats.inReview")} icon={CheckCircle} iconClass="bg-purple-subtle text-purple" href="/dashboard/briefs?status=in_review" />
          </>
        )}
      </div>
      ...
      {kpis ? (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-text">{t("dashboard.kpi.title")}</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <KpiCard label={t("dashboard.kpi.briefPending")} value={kpis.pending_briefs} icon={Clock} color="bg-warning-subtle text-warning" href="/dashboard/briefs?status=submitted" />
            <KpiCard label={t("dashboard.kpi.accepted")} value={kpis.accepted_briefs} icon={CheckCircle} color="bg-success-subtle text-success" href="/dashboard/briefs?status=accepted" />
            <KpiCard label={t("dashboard.kpi.production")} value={kpis.in_production_briefs} icon={Package} color="bg-purple-subtle text-purple" href="/dashboard/briefs?status=in_production" />
            <KpiCard label={t("dashboard.kpi.deliveryPending")} value={kpis.pending_deliverables} icon={FileText} color="bg-info-subtle text-info" />
            <KpiCard label={t("dashboard.kpi.revisionPending")} value={kpis.revision_requested_deliverables} icon={RotateCcw} color="bg-danger-subtle text-danger" />
            <KpiCard label={t("dashboard.kpi.overdueWork")} value={kpis.overdue_briefs} icon={AlertTriangle} color={kpis.overdue_briefs > 0 ? "bg-danger-subtle text-danger" : "bg-surface-2 text-text-muted"} />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
            <KpiCard label={t("dashboard.kpi.completedMonth")} value={kpis.completed_this_month} icon={CheckCircle} color="bg-success-subtle text-success" />
            <KpiCard label={t("dashboard.kpi.approvedMonth")} value={kpis.approved_this_month} icon={TrendingUp} color="bg-accent-subtle text-accent" />
            <KpiCard label={t("dashboard.kpi.openTasks")} value={kpis.open_tasks} icon={ListChecks} color="bg-purple-subtle text-purple" />
            <KpiCard label={t("dashboard.kpi.overdueTasks")} value={kpis.overdue_tasks} icon={AlertTriangle} color={kpis.overdue_tasks > 0 ? "bg-danger-subtle text-danger" : "bg-surface-2 text-text-muted"} />
          </div>
        </div>
      ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-surface border border-border rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-text text-sm">{t("dashboard.activity.title")}</h2>
            </div>
          ...
          {activity === null ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-sm font-medium text-text mb-1">{t("dashboard.activity.empty")}</p>
              <p className="text-xs text-text-muted">{t("dashboard.activity.emptyBody")}</p>
            </div>
          ) : (
            <div className="space-y-4">
              {activity.map((log) => {
                  ...
                  <div key={log.id} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-text">
                        <span className="font-medium">{actorName ?? t("dashboard.activity.someone")}</span>{" "}
                        {actionLabel(log.action, t)}{" "}
                        <span className="text-text-muted text-xs">({entityTypeLabel(log.entity_type, t)})</span>
                      </p>
                      <p className="text-xs text-text-muted mt-0.5">{formatRelativeTime(log.created_at, locale, t)}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-border rounded-xl p-5">
            <h2 className="font-semibold text-text text-sm mb-3">{t("dashboard.actions.title")}</h2>
            <div className="space-y-0.5">
              <QuickAction label={t("dashboard.actions.newBrief")} href="/dashboard/briefs/new" icon={Plus} iconClass="bg-accent/12 text-accent" />
              <QuickAction label={t("dashboard.actions.addBrand")} href="/dashboard/brands" icon={Building2} iconClass="bg-emerald-500/12 text-emerald-400" />
              <QuickAction label={t("dashboard.actions.viewCalendar")} href="/dashboard/calendar" icon={Calendar} iconClass="bg-violet-500/12 text-violet-400" />
              <QuickAction label={t("dashboard.actions.inviteTeam")} href="/dashboard/settings/members" icon={Mail} iconClass="bg-amber-500/12 text-amber-400" />
            </div>
          </div>
          ...
          <div className="bg-surface border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-text text-sm">{t("dashboard.stats.awaitingApproval")}</h2>
            ...
            {pendingBriefs === null ? (
              ...
              <p className="text-xs text-text-muted py-4 text-center">{t("dashboard.approvals.empty")}</p>
            ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {brief.deadline && (
                        <p className="text-[10px] text-text-muted">
                          {t("dashboard.deadline", { date: formatLocalizedDate(brief.deadline, locale) })}
                        </p>
                      )}
                    </div>
          ...
          {stats && (
            <div className="relative bg-gradient-to-br from-accent/10 to-violet-500/5 border border-accent/15 rounded-xl p-5 overflow-hidden">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-accent">{t("dashboard.calendar.month")}</span>
              </div>
              <p className="text-3xl font-bold text-text tracking-tight">
                {formatLocalizedNumber(stats.calendar_items_this_month, locale)}
              </p>
              <p className="text-xs text-text-muted mt-0.5 mb-3">{t("dashboard.calendar.planned")}</p>
              <Link href="/dashboard/calendar" className="text-xs text-accent hover:underline">
                {t("dashboard.calendar.view")}
              </Link>
            </div>
          )}
          ...
          {kpis && (
            <div className="bg-surface border border-border rounded-xl p-5">
              <h2 className="font-semibold text-text text-sm mb-3">{t("dashboard.delivery.title")}</h2>
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-text-muted text-xs">{t("dashboard.delivery.submitted")}</span>
                  <span className="font-semibold text-text">{formatLocalizedNumber(kpis.total_deliverables_submitted, locale)}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-text-muted text-xs">{t("dashboard.delivery.approved")}</span>
                  <span className="font-semibold text-emerald-400">{formatLocalizedNumber(kpis.total_deliverables_approved, locale)}</span>
                </div>
                {kpis.total_deliverables_submitted > 0 && (
                  <div className="mt-2">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] text-text-muted">{t("dashboard.delivery.rate")}</span>
